...
<div id="levelPanel" class="level-panel"><div class="level-head"><b>Pegel / Abstand</b><span id="levelText" role="status">—</span></div><div class="level-track" role="img" aria-label="Pegelanzeige"><span id="levelFill"></span><span class="level-target" title="Zielbereich"></span></div><small>Pegel relativ, kein Schalldruckpegel und keine Abstandsmessung. Motor für den Abgleich ausschalten.</small></div>
...
(() => {
  ...
  const configKeys = ['minCps', 'maxCps', 'thresholdMultiplier', 'thresholdFloor', 'levelLow', 'levelHigh'];
  ...
  function render(next) {
    ...
    $('levelFill').style.width = Math.max(0, Math.min(100, (finite(next.levelRms) ? next.levelRms : 0) * 100)) + '%';
  ...
  $('configForm').addEventListener('submit', event => {
    ...
    if (!values.every(Number.isFinite) || values[0] < 50 || values[1] <= values[0] || values[1] > 220 || values[2] <= 0 || values[3] < 0 || values[4] <= 0 || values[5] <= values[4]) { $('configMessage').textContent='Bitte gültige Zahlen eingeben: 50 ≤ Minimum < Maximum ≤ 220, Rauschfaktor > 0, Mindestschwelle ≥ 0, Pegelband unten < oben.'; return; }
