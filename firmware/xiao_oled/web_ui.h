...
<div class="notice"><strong>Vor dem Start: Motor AUS, Abstand einstellen.</strong> Im Ruhezustand läuft der <strong>Pegelindikator</strong>: Stelle den Abstand so ein, dass „Abstand ok“ erscheint und der Balken im grünen Bereich liegt. Erst dann ist „Messung starten“ freigegeben. Nach dem Start läuft 1 Sekunde Ruhekalibrierung, dann erkennt das Gerät den Motor selbst und beginnt die Messung — Motor erst einschalten, wenn <strong>„Auto-Start bereit — Motor jetzt AN“</strong> erscheint. Während der Messung Mikrofon, Abstand und Aufbau nicht verändern.</div>
<div class="steps"><div class="step"><b>1 · Pegel prüfen</b><br>Motor aus. Abstand bis „Abstand ok“.</div><div class="step"><b>2 · 1000 Impulse</b><br>Auto-Start ab stabilem Impulsstrom, Motor dann an.</div><div class="step"><b>3 · Formen vergleichen</b><br>Überlagerung und Rohdaten prüfen.</div></div>
<section class="panel" aria-labelledby="measurementHeading">
<h2 id="measurementHeading">Messung</h2>
<div class="status-row"><div id="state" role="status" aria-live="polite">Gerätestatus wird geladen …</div><strong id="count">0 / 1000 Impulse</strong></div>
<p id="stateHelp" class="muted">Mit dem Geräte-WLAN verbinden und http://192.168.4.1 öffnen.</p>
<div id="levelPanel" class="level-panel"><div class="level-head"><b>Pegel / Abstand</b><span id="levelText" role="status">—</span></div><div class="level-track" role="img" aria-label="Pegelanzeige"><span id="levelFill"></span><span id="levelZone" class="level-target" title="Zielbereich (gruen)"></span></div><small>Pegel relativ, kein Schalldruckpegel und keine Abstandsmessung. Motor für den Abgleich ausschalten.</small></div>
<progress id="progress" value="0" max="1000" aria-label="Erkannte Impulse"></progress>
...
<div class="selection"><label for="selected">Impuls wählen</label><input id="selected" type="range" min="1" max="1000" value="1" step="1" disabled><output id="selectedNumber" for="selected">1 / 1000</output></div><p id="selectedDetails" class="muted">Einzelwerte stehen nach der vollständigen Aufnahme bereit.</p><p id="chartMessage" role="status">Wellenformen werden erst nach Abschluss geladen.</p><button id="retryWaves" hidden>Wellenformen erneut laden</button>
</section>
<section class="panel"><details><summary>Erkennung einstellen</summary><p class="muted">Nur im Ruhezustand oder nach Abschluss/Fehler änderbar. Grenzwerte passend zur erwarteten Impulsfrequenz wählen. Zu niedrige Schwellen begünstigen Doppelerkennungen, zu hohe Schwellen übersehen Impulse.</p><form id="configForm"><fieldset id="configFields" disabled><div class="config-grid"><label for="minCps">Minimum CPS<input id="minCps" name="minCps" type="number" min="50" max="199" step="any" value="50" required><small>Untergrenze 50 CPS: darunter bricht die Messung ab</small></label><label for="maxCps">Maximum CPS<input id="maxCps" name="maxCps" type="number" min="51" max="220" step="any" value="220" required></label><label for="thresholdMultiplier">Rauschfaktor<input id="thresholdMultiplier" name="thresholdMultiplier" type="number" min="3" max="30" step="any" value="6" required><small>Multiplikator des Rauschpegels</small></label><label for="thresholdFloor">Mindestschwelle<input id="thresholdFloor" name="thresholdFloor" type="number" min="0.00001" max="0.5" step="any" value="0.001" required><small>Normierte Audioamplitude</small></label><label for="levelLow">Pegelband unten<input id="levelLow" name="levelLow" type="number" min="0.0001" max="0.9" step="any" value="0.015" required><small>Untergrenze des grünen Bereichs</small></label><label for="levelHigh">Pegelband oben<input id="levelHigh" name="levelHigh" type="number" min="0.001" max="0.99" step="any" value="0.12" required><small>Obergrenze des grünen Bereichs</small></label><label for="levelLoud">Warnschwelle<input id="levelLoud" name="levelLoud" type="number" min="0.002" max="0.99" step="any" value="0.25" required><small>darüber: Warnung, dann Übersteuerung</small></label><label class="check" for="autoStart"><input id="autoStart" name="autoStart" type="checkbox" value="1" checked> Auto-Start aktiv</label></div><div class="actions"><button type="submit">Einstellungen speichern</button><span id="configMessage" role="status" class="muted"></span></div></fieldset></form></details></section>
...
<footer>Vollständig lokal · keine Cloud, keine externen Schriften oder Bibliotheken · Geräteadresse: <a href="http://192.168.4.1/">192.168.4.1</a><br>CPS und Formstatistik erscheinen erst nach Abschluss der Messung (Ziel 1000 Impulse). Nach Abschluss JSON/CSV sichern; Zurücksetzen bzw. Neustart kann die Messdaten verwerfen.</footer>
</main>
<script>
'use strict';
(() => {
  const $ = id => document.getElementById(id);
  const knownStates = ['idle', 'calibrating', 'waiting', 'warmup', 'recording', 'complete', 'failed'];
  const configKeys = ['minCps', 'maxCps', 'thresholdMultiplier', 'thresholdFloor', 'levelLow', 'levelHigh', 'levelLoud'];
  const flagKeys = ['autoStart'];
  const labels = {idle:'Bereit — Motor aus, Abstand einstellen', calibrating:'Ruhekalibrierung — Motor AUS lassen!', waiting:'Auto-Start bereit — Motor jetzt einschalten', warmup:'Vorlauf — Impulsrate wird gemessen', recording:'Messung läuft', complete:'Messung abgeschlossen — Motor kann aus', failed:'Messung fehlgeschlagen'};
  const helps = {idle:'Motor ausgeschaltet lassen. Pegelindikator beobachten und den Abstand einstellen, bis „Abstand ok“ erscheint. Erst dann ist „Messung starten“ freigegeben.', calibrating:'Etwa 1 Sekunde still bleiben. Noch NICHT einschalten.', waiting:'Jetzt den Motor einschalten. Der Start erfolgt automatisch, sobald ein stabiler Impulsstrom erkannt wird.', warmup:'Kurze Vorlaufphase: aus der gemessenen Rate wird die Fensterlänge für 1000 Impulse berechnet. Motor weiter laufen lassen.', recording:'Motor gleichmäßig laufen lassen, bis 1000 Impulse erfasst sind. Aufbau nicht verändern.', complete:'Ergebnis prüfen und herunterladen. Eine neue Messung ersetzt die bisherigen Daten.', failed:'Fehler unten prüfen. Motor ausschalten, Aufbau korrigieren und erneut starten.'};
  ...
  function controls() {
    ...
    // A run started from a badly placed microphone cannot be interpreted later,
    // so the start button stays locked until the level indicator saw the target band.
    const levelOk = !status || status.levelReady !== false || status.state !== 'idle';
    $('start').disabled = !editable || !levelOk;
    $('start').title = levelOk ? '' : 'Pegel war noch nicht im grünen Bereich: Abstand einstellen.';
    $('levelPanel').style.display = (status && status.state === 'idle') ? 'block' : 'none';
  ...
  function clearWaves() {
    ...
    $('selected').value = '1'; $('selected').max = '1000'; $('selected').disabled = true;
    $('selectedNumber').textContent = '1 / 1000';
  ...
  function render(next) {
    ...
    const target = finite(next.target) && next.target > 0 ? next.target : 1000;
    ...
    const band = ['silent','quiet','good','high','loud'].includes(next.level) ? next.level : 'silent';
    const bandText = {silent:'kein Signal — Kabel, L/R und 3V3 prüfen', quiet:'zu leise — näher heran', good:'Abstand ok', high:'grenzwertig laut — etwas weiter weg', loud:'zu laut — Übersteuerung möglich'};
    $('levelText').textContent = bandText[band];
    // Scale against the overload threshold: with a 0.12 green ceiling a /1.0 bar
    // would show the entire usable range inside the first eighth of the track.
    const span = finite(next.levelLoud) && next.levelLoud > 0 ? next.levelLoud : 1;
    const pct = value => Math.max(0, Math.min(100, (finite(value) ? value : 0) / span * 100));
    $('levelFill').style.width = pct(next.levelAmplitude) + '%';
    if (finite(next.levelLow) && finite(next.levelHigh) && next.levelHigh > next.levelLow) {
      const zone = $('levelZone');
      zone.style.left = pct(next.levelLow) + '%';
      zone.style.width = (pct(next.levelHigh) - pct(next.levelLow)) + '%';
    }
    $('levelPanel').className = 'level-panel level-' + band;
    if (next.state === 'waiting' || next.state === 'warmup') $('count').textContent = (finite(next.seen) ? next.seen : 0) + ' Impulse erkannt' + (finite(next.provisionalCps) && next.provisionalCps > 0 ? ' · ' + format(next.provisionalCps,0) + ' CPS' : '');
    ...
    if (!configDirty) {
      for (const key of configKeys) if (finite(next[key])) $(key).value = String(next[key]);
      if (typeof next.autoStart === 'boolean') $('autoStart').checked = next.autoStart;
    }
    connection(true);
  }
  ...
  $('configForm').addEventListener('change', () => { configDirty=true; $('configMessage').textContent='Ungespeicherte Änderungen'; });
  $('configForm').addEventListener('submit', event => {
    event.preventDefault();
    const values = configKeys.map(key => Number($(key).value));
    if (!values.every(Number.isFinite) || values[0] < 50 || values[1] <= values[0] || values[1] > 220 || values[2] <= 0 || values[3] < 0 || values[4] <= 0 || values[5] <= values[4] || values[6] < values[5]) { $('configMessage').textContent='Bitte gültige Zahlen eingeben: 50 ≤ Minimum < Maximum ≤ 220, Rauschfaktor > 0, Mindestschwelle ≥ 0, Pegelband unten < oben ≤ Warnschwelle.'; return; }
    const body = new URLSearchParams();configKeys.forEach((key,i)=>body.set(key,String(values[i])));
    flagKeys.forEach(key=>body.set(key,$(key).checked?'1':'0'));action('/api/config',body.toString());
  });
  let resizeFrame;
